...
const AccordionSummary = styled((props) => (
  <MuiAccordionSummary
    expandIcon={<ArrowForwardIosSharpIcon sx={{ color: "black", fontSize: "15px", fontWeight: 900 }}/>}
    {...props}
  />
))(({ theme }) => ({
        ...
        {Object.entries(columns).map(([columnId, column], index) => {
          return (
            ...
            <div className="align-center mb-4" key={columnId} >
              <Accordion 
                defaultExpanded={['1', '4'].includes(columnId) ? true : false} 
                  ...
                  {['1', '4'].includes(columnId) ? (
                    <div className='flex absolute right-2 my-2'>
                      <div className='my-0 mr-2'>
                        {column.items.filter((x) => {return x.status === "To do"}).length > 0 ? (
                          <span className="px-1.5 py-1 rounded-xl text-white bg-to-do-color my-10 mr-1 text-xs">
                            {column.items.filter((x) => {return x.status === "To do"}).reduce((sum, a) => sum + a.point, 0)}
                          </span>
                        ) : <></>}

                        {column.items.filter((x) => {return x.status === "In progress"}).length > 0 ? (
                          <span className="px-1.5 py-1 rounded-xl text-white bg-in-progress-color my-10 mr-1 text-xs">
                            {column.items.filter((x) => {return x.status === "In progress"}).reduce((sum, a) => sum + a.point, 0)}
                          </span>
                        ) : <></>}

                        {column.items.filter((x) => {return x.status === "Done"}).length > 0 ? (
                          <span className="px-1.5 py-1 rounded-xl text-white bg-done-color my-10 mr-1 text-xs">
                            {column.items.filter((x) => {return x.status === "Done"}).reduce((sum, a) => sum + a.point, 0)}
                          </span>
                        ) : <></>}
                      </div>
                      
                      <GrayButton variant="contained">
                        {column.name === "Backlog" ? "Start Sprint" : "Complete sprint"}
                      </GrayButton>
